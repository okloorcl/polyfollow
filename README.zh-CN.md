...
连续模式适合长期运行。某一轮 polling 失败时，进程会记录错误并继续下一轮。`--max-consecutive-errors 0` 表示不因为连续 polling 错误自动退出；这也是默认配置 `global.max_consecutive_errors = 0` 的行为。如果你希望连续失败多次后自动停掉，可以设置正数：

```bash
polyfollow run --paper --max-consecutive-errors 10
```

按 `Ctrl-C` 可以优雅停止，并输出最终运行汇总。
...
| 长期运行友好 | 连续 `run` 默认记录失败轮次并继续 polling |
| 环境变量私钥 | 私钥只从环境变量读取 |
...
- P2：静态 dashboard、backtesting、cooldown audit、allocation optimizer、MarketBridge context、长期运行友好的 run loop。
